...
from unet.utils.data.load_data import create_data_loaders
from unet.utils.data.load_data_kspace import create_data_loaders_kspace
from unet.utils.common.utils import save_reconstructions, ssim_loss
from unet.utils.common.utils_kspace import save_reconstructions_kspace
from unet.utils.common.loss_function import SSIMLoss
from unet.utils.model.unet_attn import Att_UNet
from unet.utils.model.varnet_ckp import VarNet
...
def load_model_varnet_pretrained(exp_dir):
    checkpoint = torch.load(exp_dir / 'model_varnet.pt', map_location=torch.device("cpu") )
    return checkpoint

def load_model(exp_dir):
    checkpoint = torch.load(exp_dir / 'best_model.tar', map_location=torch.device("cpu") )
    return checkpoint
